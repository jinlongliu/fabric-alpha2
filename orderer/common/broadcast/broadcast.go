...
func (bh *handlerImpl) Handle(srv ab.AtomicBroadcast_BroadcastServer) error {
	...
	for {
		...
		if chdr.Type == int32(cb.HeaderType_CONFIG_UPDATE) {
			...
			// src/github.com/hyperledger/fabric/orderer/configupdate/configupdate.go:104
		...
		// 发送到channel,根据不同Enqueue不同
		// solo:src/github.com/hyperledger/fabric/orderer/solo/consensus.go:73
		// 排序
